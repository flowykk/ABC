...
- Если в командной строке присутсвует ключ `-in`, это означает, что ввод данных будет происводиться через конфигурационный файл, название которого указывается после ключа. При отсутствии нужного файла, пользователю высветится сообщение об этом, иначе входные данные будут записаны в переменные `num_students` и `output_fileName`.
- В противном случае, ввод количества студентов на экзамене и путь к выходному файлу будет производиться через ключи `-out` и `-stud`.
```cpp
